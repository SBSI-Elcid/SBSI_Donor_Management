                ...
                <v-col cols="6">
                  ...
                  <v-row no-gutters>
                    <v-col cols="12">
                      ...
                      <v-text-field outlined dense v-model="model.Password" :rules="[rules.required, rules.maxLength(250)]" required></v-text-field>
                    </v-col>
                  </v-row>

                  <v-row no-gutters>
                    <v-col cols="6">
                      <v-btn depressed small color="default" @click="generatePassword"><v-icon left>mdi-form-textbox-password</v-icon>Generate password</v-btn>
                    </v-col>
                  </v-row>
                </v-col>
...
<script lang="ts">
import CreatUserAccountDto, { ICreateUserAccountDto } from '@/models/UserAccounts/CreateUserAccountDto';
...
export default class CreateUserAccount extends VueBase {
  ...
  private async save(): Promise<void> {
    ...
    try {
      ...
      await this.userAccountService.createUser(this.model);
      this.notify_success('User account successfully created.');
      this.onClose(true);
    } catch (error: any) {
