            ...
            <v-card-text>
                ...
                <v-data-table :headers="columnHeaders"
                    ...
                    <template v-slot:[`item.FullName`]="{ item }">
                        <v-btn text small color="primary" @click="onRegister(item)">
                            {{ item.FullName }}
                        </v-btn>
                    </template>
...
import { IDonorDto,DonorDto } from '../../../models/DonorRegistration/DonorDto';
    ...
    export default class ActivityDonor extends VueBase {
        ...
        protected expectedDonor: IDonorDto = new DonorDto();

        protected schedules: ISchedule = new ScheduleDto();
        ...
        protected records: Array<ActivityDonorDto> = [];
        protected pagedSearchDto: ActivityDonorPagedSearchDto = new ActivityDonorPagedSearchDto();
        protected pagedResult!: PagedSearchResultDto<ActivityDonorDto>;
        ...
        protected newDonor(item: ActivityDonorDto): void {
            this.expectedDonor.Firstname = item.FirstName;
            this.expectedDonor.Lastname = item.LastName;  
            this.expectedDonor.Middlename = item.MiddleName;  
            this.expectedDonor.BirthDate = new Date();
        }
        ...
        protected addClick(): void {
            ...
            let route = this.$router.resolve({ path: `/activitydonorregister/${id}` });
        ...
        protected async loadrecords(): Promise<void> {
            ...
            this.pagedSearchDto.ScheduleId = this.$route.params.schedule_id;
